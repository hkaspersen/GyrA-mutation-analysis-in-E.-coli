...
load_files <- function(filepath) {
  ...
  unique.files <-
    unique(gsub("(.*?[G-g]yrA)[F-R].fas", "\\1", files))
...
DNA_to_prot_file <- function(file_f, file_r) {
  ...
  filename <- file_f
  filename <- sub("(.*?[G-g]yrA)[F-R].fas", "\\1", file_f)
...
reference <- readAAStringSet("Ecoli_reference_gyrA.txt") 

setwd("") # Specify location of files
...
pdf(file = "results.pdf", height = 15, width = 12)
total_rows_per_page = 26
start_row = 1 

if(total_rows_per_page > nrow(results)){
  end_row = nrow(results)
}else {
...
for(i in 1:ceiling(nrow(results)/total_rows_per_page)){
  ...
  grid.table(results[start_row:end_row, ])
  
  start_row = end_row + 1
  
  if((total_rows_per_page + end_row) < nrow(results)){
